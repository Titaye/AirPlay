...
unsigned char* fairplay_setup(const char* message, int length, int* size)
{
	...
	if (type == SETUP_MESSAGE_TYPE)
	{
		...
		if (seq == SETUP1_MESSAGE_SEQ)
		{
			...
			response = (unsigned char*)malloc(SETUP1_RESPONSE_LENGTH);
			(*size) = SETUP1_RESPONSE_LENGTH;
		...
		else if (seq == SETUP2_MESSAGE_SEQ)
		{
			...
			response = (unsigned char*)malloc(SETUP2_RESPONSE_LENGTH);
			(*size) = SETUP2_RESPONSE_LENGTH;
